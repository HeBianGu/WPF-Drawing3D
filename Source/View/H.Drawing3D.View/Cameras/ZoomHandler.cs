...
namespace H.Drawing3D.View.Cameras
{
    ...
    internal class ZoomHandler : MouseGestureHandler
    {
        ...
        {
            Vector3D z = this.CameraLookDirection;
            z.Normalize();
            Vector3D x = Vector3D.CrossProduct(this.CameraLookDirection, this.CameraUpDirection);
        ...
        {
            if (this.Camera is not PerspectiveCamera pcamera)
            {
                return;
            }
        ...
        {
            Vector delta = e.CurrentPosition - this.LastPoint;
        ...
        {
            return this.changeFieldOfView
                ? this.Controller.IsChangeFieldOfViewEnabled && this.Controller.ActualCamera is PerspectiveCamera
                : this.Controller.IsZoomEnabled;
        }
        ...
        private void ChangeCameraDistance(double delta, Point3D zoomAround)
        {
            ...
            double f = Math.Pow(2.5, delta);
            Vector3D newRelativePosition = relativePosition * f;
            Vector3D newRelativeTarget = relativeTarget * f;
