...
namespace H.Drawing3D.View.Cameras
{
    ...
    internal class RotateHandler : MouseGestureHandler
    {
        ...
        public void RotateTurnball(Point p1, Point p2, Point3D rotateAround)
        {
            ...
            Quaternion q1 = new(this.rotationAxisX, d * delta.X);
            Quaternion q2 = new(this.rotationAxisY, d * delta.Y);
            Quaternion q = q1 * q2;
        ...
        public void RotateTurntable(Vector delta, Point3D rotateAround)
        {
            ...
            Quaternion q1 = new(up, d * delta.X);
            Quaternion q2 = new(right, d * delta.Y);
            Quaternion q = q1 * q2;
        ...
        {
            return this.changeLookAt
                ? this.Controller.CameraMode != CameraMode.FixedPosition && this.Controller.IsPanEnabled
                : this.Controller.IsRotationEnabled;
        }
        ...
        private void RotateAroundUpAndRight(Point p1, Point p2, Point3D rotateAround)
        {
            ...
            Quaternion delta1 = new(this.CameraUpDirection, -dp.X * this.RotationSensitivity);

            // Rotate around the camera right direction
            Quaternion delta2 = new(
                Vector3D.CrossProduct(this.CameraUpDirection, this.CameraLookDirection), dp.Y * this.RotationSensitivity);

            Quaternion delta = delta1 * delta2;
            RotateTransform3D rotate = new(new QuaternionRotation3D(delta));
            Vector3D relativeTarget = rotateAround - this.CameraTarget;
            Vector3D relativePosition = rotateAround - this.CameraPosition;
        ...
        private void RotateTrackball(Point p1, Point p2, Point3D rotateAround)
        {
            ...
            Quaternion delta = new(axis, -angle * this.RotationSensitivity * 5);
            RotateTransform3D rotate = new(new QuaternionRotation3D(delta));
