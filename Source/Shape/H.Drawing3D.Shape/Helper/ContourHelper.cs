...
#endif
{
    ...
    public class ContourHelper
    {
        ...
        {
            bool hasNormals = originalMesh.Normals != null && originalMesh.Normals.Count > 0;
            bool hasTextureCoordinates = originalMesh.TextureCoordinates != null && originalMesh.TextureCoordinates.Count > 0;
            ...
            // ax + by + cz + d = 0
            float l = (float)Math.Sqrt((planeNormal.X * planeNormal.X) + (planeNormal.Y * planeNormal.Y) + (planeNormal.Z * planeNormal.Z));
            ...
            out int[] triangleIndices)
        {
            ...
            ContourFacetResult facetResult = this.GetContourFacet();
            ...
            int[,] facetIndices = ResultIndices[facetResult];
            newPositions = new[]
            {
            ...
            newTextureCoordinates = this.textures != null
                ? (new[]
                {
        ...
        {
            return this.IsSideAlone(0)
                ? this.sides[0] > 0 ? ContourFacetResult.ZeroOnly : ContourFacetResult.OneAndTwo
                : this.IsSideAlone(1)
                ? this.sides[1] > 0 ? ContourFacetResult.OneOnly : ContourFacetResult.ZeroAndTwo
                : this.IsSideAlone(2)
                ? this.sides[2] > 0 ? ContourFacetResult.TwoOnly : ContourFacetResult.ZeroAndOne
                : this.AllSidesBelowContour() ? ContourFacetResult.All : ContourFacetResult.None;
        }
        ...
        {
            Point3D firstPoint = this.points[index0];
            Point3D secondPoint = this.points[index1];
            double firstSide = this.sides[index0];
            double secondSide = this.sides[index1];
            return new Point3D(
        ...
        {
            Vector3D firstPoint = this.normals[index0];
            Vector3D secondPoint = this.normals[index1];
            double firstSide = this.sides[index0];
            double secondSide = this.sides[index1];
            return new Vector3D(
                CalculatePoint(firstPoint.X, secondPoint.X, firstSide, secondSide),
        ...
        {
            Point firstTexture = this.textures[index0];
            Point secondTexture = this.textures[index1];
            double firstSide = this.sides[index0];
            double secondSide = this.sides[index1];
        ...
        private bool IsSideAlone(int index)
        {
            ...
            int firstSideIndex = getNext(index);
            int secondSideIndex = getNext(firstSideIndex);
