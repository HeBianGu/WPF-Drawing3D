...
#endif
{
    ...
    public class Polygon3D
    {
        ...
        public Polygon Flatten()
        {
...
#else
            Vector3D backward = Vector3D.CrossProduct(
#endif
                right, up);
            Matrix3D m = new(backward.X, right.X, up.X, 0, backward.Y, right.Y, up.Y, 0, backward.Z, right.Z, up.Z, 0, 0, 0, 0, 1);
            ...
            m.M42 = -offs.Y;
#else
            Point3D offs = m.Transform(this.Points[0]);
            m.OffsetX = -offs.X;
            m.OffsetY = -offs.Y;
#endif

            Polygon polygon = new() { Points = new PointCollection(this.Points.Count) };
            foreach (Point3D p in this.Points)
            {
#if SHARPDX
                var pp = Vector3D.TransformCoordinate(p, m);
#else
                Point3D pp = m.Transform(p);
#endif
...
#else
                Vector3D n = Vector3D.CrossProduct(v1, this.Points[i] - this.Points[0]);
