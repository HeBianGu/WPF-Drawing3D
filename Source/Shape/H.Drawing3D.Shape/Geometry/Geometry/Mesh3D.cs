...
namespace H.Drawing3D.Shape.Geometry.Geometry
{
    /// <summary>
    /// Represents a 3D mesh for polygon models containing faces with any number of vertices.
    /// </summary>
    public class Mesh3D : ICloneable
    {
        ...
        public Vector3D GetFaceNormal(int faceIndex)
        {
            ...
            for (int i = 0; i + 2 < m; i++)
            {
                Point3D v0 = this.Vertices[this.Faces[faceIndex][i]];
                Point3D v1 = this.Vertices[this.Faces[faceIndex][(i + 1) % m]];
                Point3D v2 = this.Vertices[this.Faces[faceIndex][(i + 2) % m]];
        ...
        public void Quadrangulate()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < m; j++)
                {
                    Point3D mp = this.FindMidpoint(this.Faces[i][j], this.Faces[i][(j + 1) % m]);
            ...
            bool sharedVertices = true, double shrinkFactor = 0.0, List<int> faceIndices = null)
        {
            ...
            if (!sharedVertices || shrink)
            {
                ...
                MeshBuilder tm = new(false, this.TextureCoordinates != null);
                int faceIndex = 0;
                foreach (int[] face in this.Faces)
                {
                    int[] vertices = new int[face.Length];
                    int j = 0;

                    Point3D centroid = this.FindCentroid(faceIndex);
                    ...
                    foreach (int v in face)
                    {
                        ...
                        Point3D vertex = this.Vertices[v];
                        if (shrink)
                        {
            ...
            else
            {
                ...
                {
                    foreach (Point uv in this.TextureCoordinates)
                    {
        ...
        {
            List<List<int>> edges = new(this.Vertices.Count);
            foreach (Point3D v in this.Vertices)
            {
