...
namespace H.Drawing3D.Shape.Geometry.Geometry
{
    ...
    public class BoundingSphere
    {
        ...
        {
            Vector3D vector = additional.center - original.center;
            ...
            Vector3D vector3 = vector * (1 / distance);
            double r1 = Math.Min(-original.Radius, distance - additional.Radius);
            double r2 = (Math.Max(original.Radius, distance + additional.Radius) - r1) * 0.5;
        ...
        public bool RayIntersection(Ray3D ray, out Point3D[] result)
        {
            ...
            if (q >= 0)
            {
                ...
                {
                    Point3D i1 = new(x1 + (dx * t1), y1 + (dy * t1), z1 + (dz * t1));
                    Point3D i2 = new(x1 + (dx * t2), y1 + (dy * t2), z1 + (dz * t2));
