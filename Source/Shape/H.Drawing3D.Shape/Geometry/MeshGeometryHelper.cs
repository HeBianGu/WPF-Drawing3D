...
// </summary>
// --------------------------------------------------------------------------------------------------------------------
using H.Drawing3D.Shape.Helper;
using System;
...
#endif
{
    ...
    public static class MeshGeometryHelper
    {
        ...
        {
            Vector3DCollection normals = new(positions.Count);
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                Point3D p0 = positions[index0];
                Point3D p1 = positions[index1];
                Point3D p2 = positions[index2];
                Vector3D u = p1 - p0;
                Vector3D v = p2 - p0;
        ...
        {
            Int32Collection edgeIndices = new();
            Dictionary<EdgeKey, Vector3D> edgeNormals = new();
            for (int i = 0; i < mesh.TriangleIndices.Count / 3; i++)
            {
                int i0 = i * 3;
                ...
                Vector3D triangleNormal = SharedFunctions.CrossProduct(p1 - p0, p2 - p0);
                ...
                for (int j = 0; j < 3; j++)
                {
                    int index0 = mesh.TriangleIndices[i0 + j];
                    int index1 = mesh.TriangleIndices[i0 + ((j + 1) % 3)];
                    Vector3D position0 = SharedFunctions.ToVector3D(mesh.Positions[index0]);
                    Vector3D position1 = SharedFunctions.ToVector3D(mesh.Positions[index1]);
                    EdgeKey edgeKey = new(position0, position1);
                    EdgeKey reverseEdgeKey = new(position1, position0);
                    if (edgeNormals.TryGetValue(edgeKey, out Vector3D value) ||
                        edgeNormals.TryGetValue(reverseEdgeKey, out value))
                    {
                        double rawDot = SharedFunctions.DotProduct(ref triangleNormal, ref value);

                        // Acos returns NaN if rawDot > 1 or rawDot < -1
                        double dot = Math.Max(-1, Math.Min(rawDot, 1));
        ...
        public static MeshGeometry3D NoSharedVertices(this MeshGeometry3D input)
        {
            ...
            for (int i = 0; i < input.TriangleIndices.Count; i += 3)
            {
                ...
                Point3D p0 = input.Positions[index0];
                Point3D p1 = input.Positions[index1];
                Point3D p2 = input.Positions[index2];
                p.Add(p0);
                p.Add(p1);
                p.Add(p2);
                ti.Add(i0);
        ...
        public static MeshGeometry3D Simplify(this MeshGeometry3D mesh, DoubleOrSingle eps)
        {
            ...
            Dictionary<int, int> dict = new(); // map position index to first occurence of same position
            for (int i = 0; i < mesh.Positions.Count; i++)
            {
                for (int j = i + 1; j < mesh.Positions.Count; j++)
                {
                    ...
                    Vector3D v = mesh.Positions[i] - mesh.Positions[j];
                    double l2 = SharedFunctions.LengthSquared(ref v);
                    if (l2 < eps)
                    {
                        dict.Add(j, i);
                    }
            ...
            Dictionary<int, int> newIndex = new(); // map old index to new index
            for (int i = 0; i < mesh.Positions.Count; i++)
            {
...
#else
            MeshGeometry3D result = new() { Positions = p, TriangleIndices = ti };
#endif
            return result;
        }
        ...
        public static string Validate(this MeshGeometry3D mesh)
        {
            ...
            for (int i = 0; i < mesh.TriangleIndices.Count; i++)
            {
                ...
                {
                    _ = sb.AppendFormat("Wrong index {0} in triangle {1} vertex {2}", index, i / 3, i % 3);
                    _ = sb.AppendLine();
                }
            }
        ...
        {
            bool hasTextureCoordinates = mesh.TextureCoordinates != null && mesh.TextureCoordinates.Count > 0;
            bool hasNormals = mesh.Normals != null && mesh.Normals.Count > 0;
            MeshBuilder meshBuilder = new(hasNormals, hasTextureCoordinates);
            ContourHelper contourHelper = new(plane, normal, mesh);
            ...
            for (int i = 0; i < mesh.TriangleIndices.Count; i += 3)
            {
                ...
                foreach (Point tc in textureCoordinates)
                {
        ...
        private static int FindConnectedSegment(IList<Point3D> segments, Point3D point, DoubleOrSingle eps)
        {
            ...
            {
                Vector3D v = point - segments[i];
                double ls0 = SharedFunctions.LengthSquared(ref v);
                if (ls0 < best)
                {
                    result = i;
                    best = ls0;
        ...
        {
            RemoveIsolatedVertices(mesh.Positions, mesh.TriangleIndices, mesh.TextureCoordinates, mesh.Normals, out Point3DCollection vertNew, out Int32Collection triNew, out PointCollection textureNew, out Vector3DCollection normalNew);
            MeshGeometry3D newMesh = new() { Positions = vertNew, TriangleIndices = triNew, TextureCoordinates = textureNew, Normals = normalNew };
            return newMesh;
        }
            ...
            out Point3DCollection verticesOut, out Int32Collection trianglesOut, out PointCollection textureOut, out Vector3DCollection normalOut)
        {
            textureOut = null;
            normalOut = null;
            List<List<int>> tracking = new(vertices.Count);
            ...
            List<int> vertToRemove = new(vertices.Count);
            for (int i = 0; i < vertices.Count; ++i)
            {
        ...
        {
            List<int> removeOutOfRangeTriangles = new();
            ...
            if (removeOutOfRangeTriangles.Count > 0)
            {
                ...
                foreach (int idx in removeOutOfRangeTriangles)
                {
