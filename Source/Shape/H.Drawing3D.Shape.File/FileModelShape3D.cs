...
namespace H.Drawing3D.Shape.File
{
    [DisplayName("管道")]
    public class FileModelShape3D : GroupShape3DBase
    {
        public Material DefaultMaterial { get; set; } = new DiffuseMaterial(Brushes.Gray);
        ...
        public override void Draw(IGroupDrawing3D drawing)
        {
            ...
            ModelImporter importer = new ModelImporter { DefaultMaterial = this.DefaultMaterial };
            this.Model3D = importer.Load(this.Source);

            foreach (Model3D item in this.Model3D.Children)
            {
                if (item is GeometryModel3D geometryModel && geometryModel.Material == null)
                {
                    geometryModel.Material = this.DefaultMaterial;
                }
            }
        }
    }
